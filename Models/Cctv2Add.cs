...
public partial class defa {
    ...
    /// cctv2Add
    /// </summary>
    public static Cctv2Add cctv2Add
    {
        get => HttpData.Get<Cctv2Add>("cctv2Add")!;
        set => HttpData["cctv2Add"] = value;
    }

    /// <summary>
    /// Page class for CCTV
    /// </summary>
    public class Cctv2Add : Cctv2AddBase
    {
        // Constructor
        public Cctv2Add(Controller controller) : base(controller)
        {
        }

        // Constructor
        public Cctv2Add() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class Cctv2AddBase : Cctv2
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "cctv2Add";
        ...
        // Page name
        public string PageName => "Cctv2Add";
        ...
        // Constructor
        public Cctv2AddBase(Controller? controller = null): this() { // DN
            if (controller != null)
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("Cctv2List"); // No matching record, return to List page // DN
                    }
                    break;
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        else if (GetPageName(returnUrl) == "Cctv2View")
        ...
        protected async Task<JsonBoolResult> AddRow(Dictionary<string, object>? rsold = null) { // DN
            ...
            try {
                // Code
                Code.SetDbValue(rsnew, Code.CurrentValue);

                // Name
                _Name.SetDbValue(rsnew, _Name.CurrentValue);

                // Loc
                Loc.SetDbValue(rsnew, Loc.CurrentValue);

                // Status
                Status.SetDbValue(rsnew, ConvertToBool(Status.CurrentValue));

                // Incident
                Incident.SetDbValue(rsnew, Incident.CurrentValue);

                // CreatedDateTime
                CreatedDateTime.SetDbValue(rsnew, ConvertToDateTime(CreatedDateTime.CurrentValue, CreatedDateTime.FormatPattern));

                // CreatedByUserID
                CreatedByUserID.SetDbValue(rsnew, CreatedByUserID.CurrentValue);

                // UpdatedDateTime
                UpdatedDateTime.SetDbValue(rsnew, ConvertToDateTime(UpdatedDateTime.CurrentValue, UpdatedDateTime.FormatPattern));

                // UpdatedByUserID
                UpdatedByUserID.SetDbValue(rsnew, UpdatedByUserID.CurrentValue);

                // Perbaikan
                Perbaikan.SetDbValue(rsnew, ConvertToBool(Perbaikan.CurrentValue));
            } catch (Exception e) {
                if (Config.Debug)
                    throw;
